// Pet core logic - migrated from src/core/pet.py

// 类型定义（直接定义在这里避免模块导入问题）
export enum PetType {
  CAT = 'cat',
  DOG = 'dog',
  DRAGON = 'dragon',
...
const CONFIG = {
  ...
  // Token 消耗对体力的影响（每 10K tokens 消耗的体力）
...
export class Pet {
  ...
  updateFromUsage(totalTokens: number, sessionInput: number = 0, sessionOutput: number = 0): UsageImpact {
    ...
    if (sessionDelta > 0) {
      ...
      const cutoff = new Date(now.getTime() - 5 * 60 * 1000); // 5分钟前
  ...
  getCurrentStateKey(): string {
    if (this.lastExpressionState === 'intense' || this.lastExpressionState === 'working') {
      return this.lastExpressionState;
    }

    if (this.state.hp <= 20) return 'low_hp';
    if (this.state.hp <= 40) return 'tired';
    if (this.state.hunger <= 20) return 'hungry';
    if (this.state.mood >= 80) return 'happy';
    return 'normal';
  }
  ...
  private feed(now: Date): InteractionResult {
    ...
    this.lastExpressionState = 'eating';
  ...
  private play(now: Date): InteractionResult {
    ...
    this.lastExpressionState = 'playing';
  ...
  private rest(now: Date): InteractionResult {
    ...
    this.lastExpressionState = 'tired';
  ...
  addExp(amount: number): number {
    ...
    if (levelUps > 0) {
      this.lastExpressionState = 'level_up';
    }
